...
class MedicineBatchModel extends Model
{
    ...
    /**
     * Get the total remaining stock across all active batches for a medicine.
     * Used by ProcurementRouter to determine if a reorder is needed.
     */
    public function getTotalStock(int $medicineId): int
    {
        $result = $this->selectSum('quantity_remaining')
            ->where('medicine_id', $medicineId)
            ->where('status', 'active')
            ->get()
            ->getRowArray();
        return (int) ($result['quantity_remaining'] ?? 0);
    }
}
